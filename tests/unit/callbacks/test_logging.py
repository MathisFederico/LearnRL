...
class TestMetric:
    ...
    def test_init_full(self):
        ...
        check.equal(
            metric.name, "reward", f"Metric name should be reward and not {metric.name}"
        )
        check.equal(
            metric.surname,
            "rwd",
...
class TestMetricList:
    ...
    def test_add_metric(self):
        ...
        expected_metric_names = [metric.name for metric in self.metrics] + [
            "exploration"
        ]
...
class TestLoggingCallbackUpdateMetrics:
    ...
    def setup(self):
        self.metric_list = MetricList(
            [Metric("metric_1"), Metric("metric_2"), Metric("metric_2")]
        )
