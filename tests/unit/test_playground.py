...
class TestPlaygroundRunStep:
    ...
    def setup_playground(self, mocker):
        ...
        mocker.patch(
            "gym.Env.step",
            return_value=(self.next_observation, self.reward, self.done, self.info),
        )
        self.env = Env()
        ...
        def handler_mocker(
            cls, reward, done, experience, reward_handler, done_handler, logs
        ):
