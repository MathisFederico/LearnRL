        ...
        for episode in range(n_episodes):
            ...
            if (episode + 1) % episodes_cycle_len == 0 or episode == n_episodes - 1:
                for metric_name, metric_avg, metric_sum in zip(
                    ...
                    sum_cycle_avg = (
                        cycle_tracker[metric_name]["sum_cycle_sum"] / cycle_episode_seen
                    )
                    avg_cycle_avg = (
                        cycle_tracker[metric_name]["avg_cycle_sum"] / cycle_episode_seen
                    )

                    logs.update(
                        {
                            f"{metric_name}_sum_cycle_sum": cycle_tracker[metric_name][
                                "sum_cycle_sum"
                            ],
                            f"{metric_name}_avg_cycle_sum": cycle_tracker[metric_name][
                                "avg_cycle_sum"
                            ],
...
def test_logging_steps_operators_(cycle_operator, metric_name):
    ...
    logging_callback = LoggingCallback(
        metrics=[
            ("reward", {"steps": cycle_operator}),
            ("loss", {"steps": cycle_operator}),
        ],
    )
    ...
    def check_function(callbacks, logs):
        ...
        if expected is not None:
            ...
            assert logged != "N/A" and np.isclose(
                logged, expected
...
def test_logging_episodes_operators_(eps_operator, cycle_operator, metric_name):
    ...
    logging_callback = LoggingCallback(
        ...
        metrics=[
            ("reward", {"episode": eps_operator, "episodes": cycle_operator}),
...
def test_display():
    ...
    for titles_on_top in (False, True):
        for verbose in range(5):
            logging_callback = Logger(titles_on_top=titles_on_top)
