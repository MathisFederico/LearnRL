...
class MetricList:
    ...
    def __add__(
        self, metric_list: Union[str, Metric, List[Metric], List[str], "MetricList"]
    ):
...
class LoggingCallback(Callback):
    ...
    def _extract_lists(metrics):
        ...
        if metrics is not None:
            if not isinstance(metrics, (tuple, list)):
                raise ValueError(
                    "Wrong metrics format. See metrics codes in documentation."
                )
            ...
            for metric in metrics:
                if isinstance(metric, (tuple, list)):
                    ...
                    if "." in metric_name:
                        ...
                        ops = {
                            key: opertor_for_all
                            for key in ("steps", "episode", "episodes")
                        }
                    else:
                        ops = {}

                step_metrics.append(metric_name)
                steps_cycle_metrics.append(
                    ".".join((metric_name, ops.get("steps", "avg")))
                )
                episode_metrics.append(
                    ".".join((metric_name, ops.get("episode", "avg")))
                )
                episodes_cycle_metrics.append(
                    ".".join((metric_name, ops.get("episodes", "avg")))
                )
        return (
            step_metrics,
            steps_cycle_metrics,
    ...
    def _update_attr(self, attr_name, last_value, operator):
        ...
        if operator == "avg":
            ...
            setattr(
                self,
                attr_name,
                previous_value + (last_value - previous_value) / new_seen,
            )
        elif operator == "sum":
    ...
    def _update_metrics_all_agents(
        self, metric_list: MetricList, target_prefix, **kwargs
    ):
        """Update the logger attributes of all agents based on a metric list"""
        for agent_id in range(self.n_agents):
            self._update_metrics(
                metric_list, target_prefix, agent_id=agent_id, **kwargs
            )
    ...
    def on_episodes_cycle_begin(self, episode, logs=None):
        self._update_metrics_all_agents(
            self.episodes_cycle_metrics,
            "episodes_cycle",
            source_prefix="episode",
            logs=logs,
            reset=True,
        )
    ...
    def on_episode_begin(self, episode, logs=None):
        self._update_metrics_all_agents(
            self.episode_metrics, "episode", logs=logs, reset=True
        )

    def on_steps_cycle_begin(self, step, logs=None):
        self._update_metrics_all_agents(
            self.steps_cycle_metrics, "steps_cycle", logs=logs, reset=True
        )

    def on_step_end(self, step, logs=None):
        agent_id = logs.get("agent_id") if self.n_agents > 1 else None
        self._update_metrics(
            self.episode_metrics, "episode", logs=logs, agent_id=agent_id
        )
        self._update_metrics_all_agents(
            self.steps_cycle_metrics, "steps_cycle", logs=logs
        )
