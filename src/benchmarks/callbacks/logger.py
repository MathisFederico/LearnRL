...
class Logger(LoggingCallback):
    ...
    def on_step_end(self, step, logs=None):
        ...
        if self.verbose >= 4:
            ...
            self._print_metrics(
                self.step_metrics, agent_id=agent_id, logs=logs, sep=sep, end=end
            )
    ...
    def on_steps_cycle_begin(self, step, logs=None):
        ...
        if self.verbose == 4 and self.titles_on_top:
            ...
            self._print_titles(
                self.step_metrics, offset=" " * len(step_text) + " |", end="\n"
            )
    ...
    def on_episode_end(self, episode, logs=None):
        ...
        if self.verbose >= 2:
            ...
            self._print_metrics(
                self.episode_only_metrics, logs=logs, sep=" | ", titles_on_top=False
            )
    ...
    def on_episodes_cycle_end(self, episode, logs=None):
        ...
        if self.verbose == 1:
            ...
            if self.titles_on_top and self.n_agents > 1:
                self._print_titles(
                    self.episodes_cycle_metrics, prefix="\n", offset=" " * 12 + "|"
                )
    ...
    def on_run_begin(self, logs=None):
        ...
        if self.titles_on_top and self.n_agents == 1 and self.verbose == 1:
            ...
            self._print_titles(
                metrics_to_print, prefix="", offset=" " * offset_len + " |", end="\n"
            )
    ...
    def _print_metrics(
        ...
        """Print a metric list"""
        titles_on_top = (
            titles_on_top if titles_on_top is not None else self.titles_on_top
        )
    ...
    def _print_metric(self, metric: Metric, metric_value, titles_on_top, **kwargs):
        ...
        if metric.name.startswith("dt_"):
            ...
            if metric_value == 0 or (
                abs(metric_value) < 1000 and abs(metric_value) > 0.001
            ):
        ...
        metric_display = (
            metric_display + (self._number_window - len(metric_display)) * " "
        )
    ...
    def _print_titles(self, metric_list: MetricList, prefix="", offset="", end=""):
        ...
        for metric in metric_list:
            if not metric.name.startswith("dt_"):
                capitalized_surname = (
                    metric.surname.capitalize()[0] + metric.surname[1:]
                )
